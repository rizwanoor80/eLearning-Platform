...
- **v1.5, cancellation mapping for slot ends (4b):** ending a slot writes `cancel_reason = 'slot_ended'` (`LessonCancelReason::SlotEnded`) on the future `reserved` lessons it cancels — `cancelled_by_parent` for a parent, admin or system end (`cancelled_by_user_id` = the actor), `cancelled_by_tutor` for a tutor's notice end (no strike). Unlike `slot_paused`, `slot_ended` does **not** free `lessons_recurring_slot_starts_at_unique`: an ended slot's dates are never regenerated. `confirmed` lessons are never auto-cancelled by a pause or an end. A tutor's notice end leaves the slot `active`/`paused` with `ended_by_user_id`, `ended_at` and `end_effective_on` set until 4c's daily run flips it to `ended`; readers must go by `status` and `end_effective_on`, not `ended_at`.
...
- **v1.5 (as built, 4e, R101):** replaces the v1.4 `lesson_id` unique, because a weekly lesson is charged up to three times and a failed attempt must stay as a row. Adds `attempt_no` (int, default 1; a single booking is always attempt 1), `unique (lesson_id, attempt_no)`, and the partial unique index `payments_one_live_per_lesson` on `(lesson_id) WHERE status <> 'failed'` — a lesson has any number of failed attempts and at most one live one (pending, captured or refunded). The gateway idempotency key for an auto-charge is `lesson:{id}:attempt:{n}`. `payment_method_id` now has its foreign key to `payment_methods` (`restrictOnDelete`; no hard-delete path exists, cards are only marked). `Lesson::payment()` is the row with the highest `attempt_no`.
...
### video_providers
`id, code (unique: daily|zoom|meet|teams|fake), name, credentials (text, encrypted array cast; null until set — Daily: api_key + webhook_secret; fake: webhook_secret), supports_embed (bool), supports_attendance_webhooks (bool), is_active (bool), updated_by (fk users, null on delete), timestamps`
- Exactly one active row (partial unique index `video_providers_one_active`). Rows are inserted by the migration, never seeded: `daily` (inactive, no credentials, embed and webhooks true) and `fake` (embed and webhooks false; active except when the migration runs in production). There is no create or delete in the admin. `zoom`, `meet` and `teams` are valid codes with no driver and cannot be activated. Activation is refused (action and model `saving` guard) for `fake` in production, a code with no driver, or a driver with missing credentials. Credentials are hidden from serialisation and write-only in the admin. `lessons.room_provider` records which provider created each room, so switching providers never breaks existing lessons. `supports_attendance_webhooks = false` makes the lesson page show "I've joined" buttons that set `tutor_joined_at` / `learner_joined_at` manually.

### video_webhook_events
`id, provider_code, event_id, type (joined|left), room_name (`lesson-<id>`), participant (tutor|learner, nullable), occurred_at, received_at` — unique (`provider_code`, `event_id`); index on `room_name`.
- One row per verified, first-seen attendance webhook; a replay inserts nothing and dispatches nothing. Written only by `VideoWebhookController`. 7c's listener applies a row to a lesson only when `lessons.room_provider = provider_code`.
