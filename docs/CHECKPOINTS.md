...
- [x] A weekly slot on Tue 17:00 Asia/Karachi generates lessons at the correct UTC times and blocks that slot in `SlotCalculator` for a Dubai parent. _(Blocking half done in 4a: active and paused slots block, ended ones do not, `ends_on`/`end_effective_on` honoured inclusive, `SlotCalculatorTest`; the generation half lands in 4c.)_ _(Generation half done in 4c, PR #18 `dd34988`: Tuesdays 17:00 Karachi generate at 12:00Z, a Dubai parent sees 16:00; Europe/London across the late-October change (10-19 16:00Z, 10-26 17:00Z); each occurrence converted on its own date via `SlotCalculator::local()`.)_
...
- [ ] Parent end: all `reserved` lessons cancelled free; `confirmed` lessons (seeded via fake gateway) follow §4. _(4b, PR #17: the `reserved` half is done and tested — cancelled free as `cancelled_by_parent` / `slot_ended`, no ledger or payment rows, `confirmed` lessons untouched. The `confirmed`-follows-§4 half is `CancelLesson`'s existing behaviour (CP3 tests); it is exercised end to end from 4e, when fake-gateway charges exist.)_ _(Done in 4e, PR #20: `AutoChargeTest` charges a weekly lesson on the fake gateway, then a parent cancel outside the window refunds, and inside it releases to the tutor, with `ledger:verify` and no generic cancelled/skipped mail.)_
...
- [ ] `recurring:charge` on a `reserved` lesson at T−48h → `confirmed` + HOLD; simulated failures at 48/36/24 → `cancelled_payment_failed`; second consecutive failure pauses the slot; a success resets the counter. _(Tests written in 4e, PR #20 `AutoChargeTest`: ladder T−48h/36h/24h, third failure `cancelled_payment_failed`, pause at two consecutive, success resets; on the fake gateway. Left unticked until the PR merges.)_
...
- [x] Trial report without the three trial fields is rejected; regular report with them is rejected. _(7e, PR #27, main `3cc49f0`: `ProgressReportTest.php` "rejects a trial report without the three trial fields and accepts it with them", "rejects a regular report that carries trial fields", "checks the trial fields against the lesson, not the request, in the action too".)_
- [x] Submitting a report releases escrow exactly once; second submit is rejected; auto-release after 72h sets the late flag. _(7e, PR #27, main `3cc49f0`: `ProgressReportTest.php` "stores the report, releases the tutor once…", "refuses a second submit and leaves the ledger exactly as the first left it", "releases once when a submit and the sweep land on the same lesson…" (sequential, the guarantee rests on the row lock and the state-machine edge); `AutoReleaseReportsTest.php` "releases a lesson at its frozen deadline and not a minute before, flagging it late", "releases once: a second run adds no ledger entry…", and the every-path zero-sum test. The parent report email box below stays for 7f (R43), although 7e already sends the report email and the trial email link.)_
- [x] Parent receives the report email within one queue cycle; the trial email contains a working weekly-slot link. _(7e + 7f, PRs #27 and #28, main `02693bd`: `ProgressReportTest.php` "puts the trial fields and a working weekly-slot link in the trial email…" and the queued-listener tests (the mail is handed to the queue and the account holder only receives it; delivery itself is not proven, R43); `TrialFollowUpTest.php` "puts a link in the trial email that opens the pre-filled form for the recipient" and the twelve 7f tests for the pre-filled form and the learner timeline.)_
