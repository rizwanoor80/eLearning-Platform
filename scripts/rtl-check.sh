...
# Optional first argument: the directory to check (default resources/) — lets
# tests/Feature/RtlCheckTest.php (R24) point this at throwaway fixtures
...
violations=$(grep -rInE "$PATTERN" "$TARGET" \
...
echo "RTL check passed: no physical-direction utilities found in $TARGET (excluding the animate plugin's fixed slide-in-from-left/right keyframe names, R9)."
